  ...
  <div>
    ...
    <div class="hy-body-title">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item>
          优惠券主题
          <router-link tag="span" to="/Web/wxCoupon/CouponThemeList/Add">
            <el-button size="small" icon="el-icon-circle-plus" type="danger">添加主题</el-button>
          </router-link>
          <!-- <el-button size="small" @click="SetFlower">
            <router-link tag="a" to="/Web/wxCoupon/CouponThemeList/Add">添加主题</router-link>
          </el-button>-->
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      <el-table :data="listData" stripe style="width: 100%">
        ...
        <el-table-column label="主题图">
          <template slot-scope="scope">
            <img :src="scope.row.ImgUrl" style="max-width:200px" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="text" size="small">
              <router-link
                tag="span"
                :to="'/Web/wxCoupon/CouponThemeList/edit?id='+scope.row.ZoneId"
              >查看详情</router-link>
            </el-button>
...
export default {
  ...
  methods: {
    ...
    queryList() {
      ...
      that.$axios.post(QueryCouponZoneList, params).then(ret => {
        that.loading = false;
    ...
    itemDel(zoneId) {
      ...
      that
        ...
        .then(() => {
          ...
          that.$axios.post(DelCouponZone, params).then(ret => {
            if (ret.Code == "200") {
              that.queryList();
            }
          });
