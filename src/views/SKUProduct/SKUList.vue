 ...
 * @LastEditTime: 2020-12-24 11:50:00
 -->
<template>
  <div>
    ...
    <div class="hy-search-box">
      <el-form :inline="true" :model="searchData" size="small" @submit.native.prevent class="demo-form-inline">
    ...
    <div class="hy-body-centent" v-loading="loading">
      <el-table :data="listData" stripe style="width: 100%">
        ...
        <el-table-column label="图片" width="200" align="center">
          <template slot-scope="scope">
            <!--  + '?imageView2/2/w/100/h/100/q/100|imageslim' -->
            <img v-if="scope.row.SmImg" :src="scope.row.SmImg" class="el-Flower-picture" />

            <span>
              <template v-if="!scope.row.input">
                <hyUpload :uploadData="{ index: scope.$index, uploadURL: 'commodity/SKU' }" @change="successUpload"></hyUpload>
                <el-input v-model="scope.row.SmImg" placeholder="输入链接"></el-input>
              </template>
            </span>
          </template>
        </el-table-column>
        <el-table-column label="基本信息" width="240">
          <template slot-scope="scope">
            <p class="input-p">库存：<el-input type="Number" :disabled="scope.row.input" style="width:140px !important" v-model="scope.row.Stock" autocomplete="off"></el-input></p>
            <p class="input-p">销量：<el-input type="Number" :disabled="scope.row.input" style="width:140px !important" v-model="scope.row.Sales" autocomplete="off"></el-input></p>
          </template>
        </el-table-column>
        <el-table-column label="价格设置" width="240">
          <template slot-scope="scope">
            <p class="input-p">成本价：<el-input type="Number" :disabled="scope.row.input" style="width:140px !important" v-model="scope.row.CostPrice" autocomplete="off"></el-input></p>
            <p class="input-p">销售价：<el-input type="Number" :disabled="scope.row.input" style="width:140px !important" v-model="scope.row.SalePrice" autocomplete="off"></el-input></p>
            <p class="input-p">市场价：<el-input type="Number" :disabled="scope.row.input" style="width:140px !important" v-model="scope.row.MarketPrice" autocomplete="off"></el-input></p>
          </template>
        </el-table-column>
        <el-table-column label="分销价格设置" width="240">
          <template slot-scope="scope">
            ...
            <p class="input-p">分销提成：<el-input type="Number" :disabled="scope.row.input" style="width:140px !important" v-model="scope.row.FirstRebate" autocomplete="off"></el-input></p>
            <p class="input-p">上级提成：<el-input type="Number" :disabled="scope.row.input" style="width:140px !important" v-model="scope.row.ParentRebate" autocomplete="off"></el-input></p>
          </template>
        </el-table-column>
